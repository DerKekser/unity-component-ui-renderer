...
namespace Kekser.ComponentSystem.StyleGenerator
{
    public class StyleGenerator
    {
        ...
        private static string[] ExtractStringsFromScripts(string[] paths)
        {
            ...
            string searchPath = "Assets\\Examples";
        ...
        private static StyleRule[] _rules = new []
        {
            ...
            new StyleRule(@"^mt-\[(.*?)]$", (match, className) => $".{className} {{ margin-top: {match.Groups[1].Value}; }}"),
            new StyleRule("^mr-([0-9]+)$", (match, className) => $".{className} {{ margin-right: {match.Groups[1].Value}px; }}"),
            new StyleRule(@"^mr-\[(.*?)]$", (match, className) => $".{className} {{ margin-right: {match.Groups[1].Value}; }}"),
            new StyleRule("^mb-([0-9]+)$", (match, className) => $".{className} {{ margin-bottom: {match.Groups[1].Value}px; }}"),
            new StyleRule(@"^mb-\[(.*?)]$", (match, className) => $".{className} {{ margin-bottom: {match.Groups[1].Value}; }}"),
            new StyleRule("^ml-([0-9]+)$", (match, className) => $".{className} {{ margin-left: {match.Groups[1].Value}px; }}"),
            new StyleRule(@"^ml-\[(.*?)]$", (match, className) => $".{className} {{ margin-left: {match.Groups[1].Value}; }}"),
            
            new StyleRule("^p-([0-9]+)$", (match, className) => $".{className} {{ padding: {match.Groups[1].Value}px; }}"),
            new StyleRule(@"^p-\[(.*?)]$", (match, className) => $".{className} {{ padding: {match.Groups[1].Value}; }}"),
            new StyleRule("^pt-([0-9]+)$", (match, className) => $".{className} {{ padding-top: {match.Groups[1].Value}px; }}"),
            new StyleRule(@"^pt-\[(.*?)]$", (match, className) => $".{className} {{ padding-top: {match.Groups[1].Value}; }}"),
            new StyleRule("^pr-([0-9]+)$", (match, className) => $".{className} {{ padding-right: {match.Groups[1].Value}px; }}"),
            new StyleRule(@"^pr-\[(.*?)]$", (match, className) => $".{className} {{ padding-right: {match.Groups[1].Value}; }}"),
            new StyleRule("^pb-([0-9]+)$", (match, className) => $".{className} {{ padding-bottom: {match.Groups[1].Value}px; }}"),
            new StyleRule(@"^pb-\[(.*?)]$", (match, className) => $".{className} {{ padding-bottom: {match.Groups[1].Value}; }}"),
            new StyleRule("^pl-([0-9]+)$", (match, className) => $".{className} {{ padding-left: {match.Groups[1].Value}px; }}"),
            new StyleRule(@"^pl-\[(.*?)]$", (match, className) => $".{className} {{ padding-left: {match.Groups[1].Value}; }}"),
            
            new StyleRule("^w-([0-9]+)$", (match, className) => $".{className} {{ width: {match.Groups[1].Value}px; }}"),
            new StyleRule(@"^w-\[(.*?)]$", (match, className) => $".{className} {{ width: {match.Groups[1].Value}; }}"),
            new StyleRule("^h-([0-9]+)$", (match, className) => $".{className} {{ height: {match.Groups[1].Value}px; }}"),
            new StyleRule(@"^h-\[(.*?)]$", (match, className) => $".{className} {{ height: {match.Groups[1].Value}; }}"),
            
            new StyleRule("^max-w-([0-9]+)$", (match, className) => $".{className} {{ max-width: {match.Groups[1].Value}px; }}"),
            new StyleRule(@"^max-w-\[(.*?)]$", (match, className) => $".{className} {{ max-width: {match.Groups[1].Value}; }}"),
            new StyleRule("^max-h-([0-9]+)$", (match, className) => $".{className} {{ max-height: {match.Groups[1].Value}px; }}"),
            new StyleRule(@"^max-h-\[(.*?)]$", (match, className) => $".{className} {{ max-height: {match.Groups[1].Value}; }}"),
            new StyleRule("^min-w-([0-9]+)$", (match, className) => $".{className} {{ min-width: {match.Groups[1].Value}px; }}"),
            new StyleRule(@"^min-w-\[(.*?)]$", (match, className) => $".{className} {{ min-width: {match.Groups[1].Value}; }}"),
            new StyleRule("^min-h-([0-9]+)$", (match, className) => $".{className} {{ min-height: {match.Groups[1].Value}px; }}"),
            new StyleRule(@"^min-h-\[(.*?)]$", (match, className) => $".{className} {{ min-height: {match.Groups[1].Value}; }}"),
            
            new StyleRule("^t-([0-9]+)$", (match, className) => $".{className} {{ top: {match.Groups[1].Value}px; }}"),
            new StyleRule(@"^t-\[(.*?)]$", (match, className) => $".{className} {{ top: {match.Groups[1].Value}; }}"),
            new StyleRule("^r-([0-9]+)$", (match, className) => $".{className} {{ right: {match.Groups[1].Value}px; }}"),
            new StyleRule(@"^r-\[(.*?)]$", (match, className) => $".{className} {{ right: {match.Groups[1].Value}; }}"),
            new StyleRule("^b-([0-9]+)$", (match, className) => $".{className} {{ bottom: {match.Groups[1].Value}px; }}"),
            new StyleRule(@"^b-\[(.*?)]$", (match, className) => $".{className} {{ bottom: {match.Groups[1].Value}; }}"),
            new StyleRule("^l-([0-9]+)$", (match, className) => $".{className} {{ left: {match.Groups[1].Value}px; }}"),
            new StyleRule(@"^l-\[(.*?)]$", (match, className) => $".{className} {{ left: {match.Groups[1].Value}; }}"),
            
            new StyleRule("^translate-([0-9]+)$", (match, className) => $".{className} {{ translate: {match.Groups[1].Value}px {match.Groups[1].Value}px; }}"),
            new StyleRule(@"^translate-\[(.*?)]$", (match, className) => $".{className} {{ translate: {match.Groups[1].Value} {match.Groups[1].Value}; }}"),
            new StyleRule("^translateX-([0-9]+)$", (match, className) => $".{className} {{ translate: {match.Groups[1].Value}px 0px; }}"),
            new StyleRule(@"^translateX-\[(.*?)]$", (match, className) => $".{className} {{ translate: {match.Groups[1].Value} 0px; }}"),
            new StyleRule("^translateY-([0-9]+)$", (match, className) => $".{className} {{ translate: 0px {match.Groups[1].Value}px; }}"),
            ...
            new StyleRule("^flex-([0-9]+)$", (match, className) => $".{className} {{ flex: {match.Groups[1].Value}; }}"),
            new StyleRule(@"^flex-\[(.*?)]$", (match, className) => $".{className} {{ flex: {match.Groups[1].Value}; }}"),
            new StyleRule("^flex-grow-([0-9]+)$", (match, className) => $".{className} {{ flex-grow: {match.Groups[1].Value}; }}"),
            new StyleRule(@"^flex-grow-\[(.*?)]$", (match, className) => $".{className} {{ flex-grow: {match.Groups[1].Value}; }}"),
            new StyleRule("^flex-shrink-([0-9]+)$", (match, className) => $".{className} {{ flex-shrink: {match.Groups[1].Value}; }}"),
            new StyleRule(@"^flex-shrink-\[(.*?)]$", (match, className) => $".{className} {{ flex-shrink: {match.Groups[1].Value}; }}"),
            new StyleRule("^flex-basis-([0-9]+)$", (match, className) => $".{className} {{ flex-basis: {match.Groups[1].Value}px; }}"),
            new StyleRule(@"^flex-basis-\[(.*?)]$", (match, className) => $".{className} {{ flex-basis: {match.Groups[1].Value}; }}"),
